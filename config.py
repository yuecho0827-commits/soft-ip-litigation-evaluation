...
LEGACY_DB_CANDIDATES = [
    LEGACY_RUNTIME_DIR / "soft_ip.db",
    LEGACY_DATA_DIR / "soft_ip.db",
    BASE_DIR / "soft_ip_runtime.db",
]
LEGACY_REPORT_DIRS = [
    LEGACY_RUNTIME_DIR / "reports",
    LEGACY_DATA_DIR / "reports",
]
...
def get_runtime_settings() -> dict:
    ...
    return {
        ...
        "qcc_api_token": qcc_api_token,
        "pkulaw_api_token": pkulaw_api_token,
        "env_path": str(ENV_PATH),
        "api_key_configured": bool(deepseek_api_key),
        "qcc_api_token_configured": bool(qcc_api_token),
        "pkulaw_api_token_configured": bool(pkulaw_api_token),
    }
...
# 数据库配置（优先使用用户目录文件数据库，失败时回退到内存数据库）
try:
    RUNTIME_DIR.mkdir(parents=True, exist_ok=True)
...
def save_runtime_settings(
    *,
    ...
    qcc_api_token: str,
    pkulaw_api_token: str,
) -> dict:
    values = _read_env_file()
    values.update({
        "USE_MOCK": "True" if use_mock else "False",
        "LLM_PROVIDER": (llm_provider or "deepseek").strip() or "deepseek",
        "DEEPSEEK_API_KEY": (deepseek_api_key or "").strip(),
        "DEEPSEEK_BASE_URL": (deepseek_base_url or "https://api.deepseek.com").strip() or "https://api.deepseek.com",
        "QCC_API_TOKEN": (qcc_api_token or "").strip(),
        "PKULAW_API_TOKEN": (pkulaw_api_token or "").strip(),
    })
...
def get_runtime_configuration_status():
    ...
    return {
        ...
        "mode_label": "Mock 模拟模式" if settings["use_mock"] else "真实 Demo 模式",
        "ready": settings["use_mock"] or not missing_required,
        "missing_required": missing_required,
        "missing_optional_services": missing_optional_services,
        "optional_warnings": optional_warnings,
        "configured_items": {
            "USE_MOCK": str(settings["use_mock"]),
            "LLM_PROVIDER": settings["llm_provider"],
            "DEEPSEEK_BASE_URL": settings["deepseek_base_url"] or "(default)",
            "DEEPSEEK_API_KEY": "已配置" if settings["deepseek_api_key"].strip() else "未配置",
            "QCC_API_TOKEN": "已配置" if settings["qcc_api_token"].strip() else "未配置",
            "PKULAW_API_TOKEN": "已配置" if settings["pkulaw_api_token"].strip() else "未配置",
            "DB_PATH": str(DB_PATH),
        ...
        "env_path": str(ENV_PATH),
        "user_data_dir": str(USER_DATA_DIR),
        "preferred_user_data_dir": str(PREFERRED_USER_DATA_DIR),
        "install_dir": str(BASE_DIR),
        "report_dir": str(REPORT_DIR),
        "using_fallback_storage": USER_DATA_DIR == FALLBACK_USER_DATA_DIR,
        "storage_notice": storage_notice,
        "llm_provider": settings["llm_provider"],
        "deepseek_base_url": settings["deepseek_base_url"],
        "api_key_configured": settings["api_key_configured"],
        "qcc_api_token_configured": settings["qcc_api_token_configured"],
        "pkulaw_api_token_configured": settings["pkulaw_api_token_configured"],
    }
