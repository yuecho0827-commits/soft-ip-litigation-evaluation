...
def _build_conclusion_section(score_result: Dict, data_integrity: Dict, critical_issues: List, optional_issues: List) -> str:
    """一、结论摘要"""
    md = f"**总体建议**: {score_result.get('recommendation', '待评估')}\n"
    md += f"**综合评分**: {_display_score(score_result.get('final_score'), ' 分（满分100分）')}\n"
    md += f"**置信度**: {_display_score(score_result.get('confidence_score'), '%')}\n"
    md += f"**数据完整性**: {data_integrity.get('label', '完整' if data_integrity.get('is_complete', True) else '部分完成')}\n\n"
    md += f"{score_result.get('reason', '')}\n\n"
    if critical_issues:
...
def _build_scoring_section(score_result: Dict) -> str:
    """二、三维评分总览"""
    md = '| 维度 | 得分 | 权重 | 说明 |\n|------|------|------|------|\n'
...
def _build_rules_section(rule_results: List) -> str:
    """三、红线风险检查"""
    md = ''
...
def _build_legal_analysis_section(legal_analysis: Dict) -> str:
    """四、法律要件分析"""
    md = ''
...
def _build_evidence_section(rule_results: List) -> str:
    """五、证据缺口诊断"""
    md = '（详细证据矩阵请在系统中查看）\n\n**关键缺失证据**:\n'
    for rule in rule_results:
...
def _build_appendix_section(now: str) -> str:
    """六、附录"""
    return (
        '**评估模型版本**: v0.1.0 MVP\n'
        '**评估方法**: 规则引擎 + LLM 辅助分析\n'
...
def generate_markdown_report(case_info: Dict, score_result: Dict, rule_results: List, legal_analysis: Dict) -> str:
    ...
    md = (
        f"# Soft IP 主诉评估报告\n\n"
        f"**案件名称**: {case_info.get('name', '未命名案件')}\n"
        f"**评估时间**: {now}\n"
        f"**案由**: {case_info.get('cause_type', '商标侵权')}\n"
        f"**业务目标**: {case_info.get('goal_type', '未指定')}\n"
        f"**我司主体**: {case_info.get('client_org', '未指定')}\n\n"
        f"---\n\n"
        f"## 一、结论摘要\n\n"
        f"{_build_conclusion_section(score_result, data_integrity, critical_issues, optional_issues)}"
        f"\n---\n\n"
        f"## 二、三维评分总览\n\n"
        f"{_build_scoring_section(score_result)}"
        f"---\n\n"
        f"## 三、红线风险检查\n\n"
        f"{_build_rules_section(rule_results)}"
        f"---\n\n"
        f"## 四、法律要件分析\n\n"
        f"{_build_legal_analysis_section(legal_analysis)}"
        f"---\n\n"
        f"## 五、证据缺口诊断\n\n"
        f"{_build_evidence_section(rule_results)}"
        f"\n---\n\n"
        f"## 六、附录\n\n"
        f"{_build_appendix_section(now)}"
    )
    return md

def _register_chinese_font() -> str:
    """按平台尝试注册中文字体，失败时退回内置中文字体。"""
    font_candidates = ['C:\\Windows\\Fonts\\simhei.ttf', 'C:\\Windows\\Fonts\\msyh.ttc', 'C:\\Windows\\Fonts\\simsun.ttc', '/usr/share/fonts/truetype/wqy/wqy-zenhei.ttc', '/usr/share/fonts/truetype/droid/DroidSansFallbackFull.ttf', '/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc', '/System/Library/Fonts/STHeiti Light.ttc', '/System/Library/Fonts/STHeiti Medium.ttc', '/System/Library/Fonts/Supplemental/Songti.ttc']
...
def generate_pdf_bytes(markdown_content: str) -> bytes:
    ...
    title_style = ParagraphStyle('ChineseTitle', fontName=cn_font_name, fontSize=18, textColor=colors.HexColor('#1a1a1a'), spaceAfter=20, alignment=TA_CENTER, leading=24)
    h1_style = ParagraphStyle('ChineseH1', fontName=cn_font_name, fontSize=14, textColor=colors.HexColor('#2c3e50'), spaceAfter=10, spaceBefore=16, leading=20)
    h2_style = ParagraphStyle('ChineseH2', fontName=cn_font_name, fontSize=12, textColor=colors.HexColor('#34495e'), spaceAfter=8, spaceBefore=12, leading=18)
    body_style = ParagraphStyle('ChineseBody', fontName=cn_font_name, fontSize=10, textColor=colors.HexColor('#333333'), leading=16, spaceAfter=4)
    story = []
    ...
    for line in lines:
        ...
        if stripped.startswith('# ') and (not stripped.startswith('## ')):
            ...
            if cells:
                ...
                table.setStyle(TableStyle([('FONTNAME', (0, 0), (-1, -1), cn_font_name), ('FONTSIZE', (0, 0), (-1, -1), 9), ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#f0f0f0')), ('GRID', (0, 0), (-1, -1), 0.5, colors.HexColor('#cccccc')), ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'), ('TOPPADDING', (0, 0), (-1, -1), 4), ('BOTTOMPADDING', (0, 0), (-1, -1), 4)]))
                story.append(table)
        elif stripped.startswith('- '):
...
if __name__ == '__main__':
    ...
    test_score = {'recommendation': '建议起诉', 'final_score': 75, 'confidence_score': 70, 'data_integrity': {'is_complete': True, 'label': '完整'}}
    test_rules = []
