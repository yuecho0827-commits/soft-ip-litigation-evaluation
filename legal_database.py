...
def generate_search_queries(case_description: str) -> List[Dict]:
    ...
    if any(k in case_description for k in ["商标", "注册商标"]):
        ...
        queries.append({
            ...
            "text": f"商标侵权 近似商标 混淆",
            "case_type": "民事案件",
            "description": "商标侵权类案检索"
        })
        queries.append({
            "type": "law_search",
            "text": "商标侵权 损害赔偿 法定赔偿",
            "description": "商标侵权赔偿标准"
        })
    ...
    if "驰名商标" in case_description or "驰名" in case_description:
        queries.append({
            "type": "law_keyword",
            "title": "驰名商标认定和保护规定",
            "description": "驰名商标相关法规"
        })
        queries.append({
            "type": "case_search",
            "text": f"驰名商标 跨类保护",
            "case_type": "民事案件",
            "description": "驰名商标保护类案"
        })
    ...
    if any(k in case_description for k in ["赔偿", "损失", "获利", "销售额", "损害"]):
        queries.append({
            "type": "case_search",
            "text": f"商标侵权 赔偿额 法定赔偿",
            "case_type": "民事案件",
            "description": "商标侵权赔偿额参考案例"
        })

    if any(k in case_description for k in ["公证", "证据", "固定证据"]):
        queries.append({
            "type": "case_search",
            "text": f"商标侵权 电子证据 公证取证",
            "case_type": "民事案件",
            "description": "证据采信标准参考案例"
        })
