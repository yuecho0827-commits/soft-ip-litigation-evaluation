...
from services.api_client import require_token, bearer_headers, skipped_result as _shared_skipped
API_BASE = 'https://agent.qcc.com/mcp'
TOOL_SERVER = {'get_company_by_query': 'company', 'get_company_profile': 'company', 'get_company_registration_info': 'company', 'get_financial_data': 'company', 'get_listing_info': 'company', 'get_branches': 'company', 'get_external_investments': 'company', 'get_actual_controller': 'company', 'get_key_personnel': 'company', 'get_annual_reports': 'company', 'get_change_records': 'company', 'get_contact_info': 'company', 'verify_company_accuracy': 'company', 'get_company_risk_scan': 'risk', 'get_dishonest_info': 'risk', 'get_judgment_debtor_info': 'risk', 'get_high_consumption_restriction': 'risk', 'get_terminated_cases': 'risk', 'get_business_exception': 'risk', 'get_serious_violation': 'risk', 'get_equity_freeze': 'risk', 'get_cancellation_record_info': 'risk', 'get_liquidation_info': 'risk', 'get_bankruptcy_reorganization': 'risk', 'get_chattel_mortgage_info': 'risk', 'get_land_mortgage_info': 'risk', 'get_equity_pledge_info': 'risk', 'get_stock_pledge_info': 'risk', 'get_guarantee_info': 'risk', 'get_judicial_auction': 'risk', 'get_case_filing_info': 'risk', 'get_court_notice': 'risk', 'get_tax_arrears_notice': 'risk', 'get_tax_violation': 'risk', 'get_tax_abnormal': 'risk', 'get_administrative_penalty': 'risk', 'get_environmental_penalty': 'risk', 'get_disciplinary_list': 'risk', 'get_default_info': 'risk', 'get_judicial_documents': 'risk', 'get_company_related_risk_scan': 'risk', 'get_property_asset_announcement': 'risk', 'get_exit_restriction': 'risk', 'get_executive_risk_scan': 'executive', 'get_executive_dishonest': 'executive', 'get_executive_judgment_debtor': 'executive', 'get_executive_equity_freeze': 'executive', 'get_executive_equity_pledge': 'executive', 'get_executive_high_consumption_ban': 'executive', 'get_executive_property_reward_notice': 'executive', 'get_executive_exit_restriction': 'executive', 'get_executive_tax_violation': 'executive', 'get_executive_admin_penalty': 'executive', 'get_executive_controlled_companies': 'executive', 'get_executive_investments': 'executive', 'get_executive_related_risk_scan': 'executive', 'get_executive_case_filing': 'executive', 'get_trademark_info': 'ipr', 'get_online_store': 'ipr', 'get_app_info': 'ipr', 'get_mini_program': 'ipr', 'get_wechat_official_account': 'ipr', 'get_douyin_account': 'ipr', 'get_ipr_pledge': 'ipr', 'get_bidding_info': 'operation', 'get_financing_records': 'operation', 'get_honor_info': 'operation', 'get_ranking_list_info': 'operation', 'get_recruitment_info': 'operation', 'get_credit_evaluation': 'operation', 'get_administrative_license': 'operation', 'get_qualifications': 'operation', 'get_judicial_case_search': 'case'}

def _qcc_configured() -> bool:
    _, _, ok = require_token('qcc_api_token')
    return ok

def _qcc_headers() -> dict:
    token, err, ok = require_token('qcc_api_token')
    if not ok:
        raise RuntimeError(err or '未配置 QCC_API_TOKEN，无法调用企查查服务')
    return bearer_headers(token)

def _qcc_unconfigured_result() -> Dict:
    return _shared_skipped('未配置 QCC_API_TOKEN，企查查画像已跳过', defaults={'stages': {}, 'metrics': {}})
...
def _safe_call(tool_name: str, search_key: str, server: str=None, extra_args: dict=None, search_param: str='searchKey') -> dict:
    """安全包装调用"""
    args = {search_param: search_key}
    if extra_args:
...
def _search_enterprise(d: dict) -> Dict:
    ...
    result = {'_summary': '', 'stages': {}, 'metrics': {'recovery_probability': 50.0, 'damages_p50': None, 'time_extra_months': 0, 'red_flags': [], 'green_flags': []}}
    summary_parts = []
...
def _stage_b_basics(locked_name: str) -> dict:
    """并行获取基本盘"""
    result = {}
    calls = [('工商登记', 'get_company_registration_info', 'company'), ('企业简介', 'get_company_profile', 'company'), ('财务数据', 'get_financial_data', 'company'), ('上市信息', 'get_listing_info', 'company'), ('分支机构', 'get_branches', 'company'), ('对外投资', 'get_external_investments', 'company'), ('实际控制人', 'get_actual_controller', 'company'), ('核心人员', 'get_key_personnel', 'company'), ('年报', 'get_annual_reports', 'company'), ('近一年变更', 'get_change_records', 'company')]
    for label, tool, server in calls:
        r = _safe_call(tool, locked_name, server)
        items = _extract_items(r)
        result[label] = {'_count': len(items), '_summary': f'{len(items)}条记录' if items else '无数据', '_items': items[:3]}
    return result
...
def _stage_c_risk_scan(locked_name: str) -> dict:
    ...
    return {'hits': hits, '_summary': f'命中 {len(hits)} 个风险维度, 共 {int(total_hits)} 条' if total_hits > 0 else '未命中风险维度', 'total_hit_dimensions': len([k for k in hits if k != '_raw'])}

def _stage_d_risk_drill(locked_name: str, scan_hits: dict) -> dict:
    """按风险分诊命中结果下钻"""
    result = {}
    p0_tools = [('注销记录', 'get_cancellation_record_info'), ('清算信息', 'get_liquidation_info'), ('破产重整', 'get_bankruptcy_reorganization')]
    ...
    p1_tools = [('被执行人', 'get_judgment_debtor_info'), ('失信信息', 'get_dishonest_info'), ('终本案件', 'get_terminated_cases'), ('限高消费', 'get_high_consumption_restriction'), ('严重违法', 'get_serious_violation'), ('经营异常', 'get_business_exception')]
    for label, tool in p1_tools:
        r = _safe_call(tool, locked_name, server='risk')
        result[label] = {'_count': _count_items(r), '_summary': f'{_count_items(r)}条'}
    p2_tools = [('股权冻结', 'get_equity_freeze'), ('动产抵押', 'get_chattel_mortgage_info'), ('土地抵押', 'get_land_mortgage_info'), ('股权出质', 'get_equity_pledge_info'), ('股票质押', 'get_stock_pledge_info'), ('司法拍卖', 'get_judicial_auction'), ('担保信息', 'get_guarantee_info'), ('财产悬赏', 'get_property_asset_announcement')]
    for label, tool in p2_tools:
        r = _safe_call(tool, locked_name, server='risk')
        c = _count_items(r)
        if c > 0:
            result[label] = {'_count': c, '_summary': f'{c}条 ⚠️'}
    p3_tools = [('欠税公告', 'get_tax_arrears_notice'), ('税收违法', 'get_tax_violation'), ('税务异常', 'get_tax_abnormal'), ('行政处罚', 'get_administrative_penalty'), ('环保处罚', 'get_environmental_penalty'), ('惩戒名单', 'get_disciplinary_list'), ('违约信息', 'get_default_info')]
    for label, tool in p3_tools:
        r = _safe_call(tool, locked_name, server='risk')
        c = _count_items(r)
        if c > 0:
            result[label] = {'_count': c, '_summary': f'{c}条'}
    p4_tools = [('裁判文书', 'get_judicial_documents'), ('法院立案', 'get_case_filing_info'), ('法院公告', 'get_court_notice'), ('限制出境', 'get_exit_restriction')]
...
def _stage_e_executive_risk(locked_name: str, controller: dict, personnel: dict) -> dict:
    ...
    for person in people[:3]:
        ...
        scan_r = _safe_call('get_executive_risk_scan', pname, server='executive', extra_args={'personName': pname})
        person_result['scans']['风险分诊'] = {'_count': _count_items(scan_r)}
        drill_tools = [('失信', 'get_executive_dishonest'), ('被执行', 'get_executive_judgment_debtor'), ('限高', 'get_executive_high_consumption_ban'), ('股权冻结', 'get_executive_equity_freeze'), ('股权出质', 'get_executive_equity_pledge')]
...
def _stage_f_business_scale(locked_name: str) -> dict:
    """经营规模 + 侵权渠道"""
    result = {}
    ipr_tools = [('商标资产', 'get_trademark_info', 'ipr'), ('线上店铺', 'get_online_store', 'ipr'), ('APP信息', 'get_app_info', 'ipr'), ('小程序', 'get_mini_program', 'ipr'), ('微信公众号', 'get_wechat_official_account', 'ipr'), ('抖音账号', 'get_douyin_account', 'ipr')]
    for label, tool, server in ipr_tools:
        r = _safe_call(tool, locked_name, server)
        c = _count_items(r)
        result[label] = {'_count': c, '_summary': f'{c}条'}
    op_tools = [('招投标', 'get_bidding_info', 'operation'), ('融资记录', 'get_financing_records', 'operation'), ('荣誉信息', 'get_honor_info', 'operation'), ('榜单排名', 'get_ranking_list_info', 'operation'), ('招聘信息', 'get_recruitment_info', 'operation'), ('信用评估', 'get_credit_evaluation', 'operation'), ('行政许可', 'get_administrative_license', 'operation'), ('资质证书', 'get_qualifications', 'operation')]
    for label, tool, server in op_tools:
        r = _safe_call(tool, locked_name, server)
        c = _count_items(r)
        result[label] = {'_count': c, '_summary': f'{c}条'}
    total_channels = sum((result[k]['_count'] for k in result if k in ('线上店铺', 'APP信息', '小程序', '微信公众号', '抖音账号')))
...
def _stage_g_litigation_timeline(locked_name: str) -> dict:
    """诉讼时间预估"""
    result = {}
    r = _safe_call('get_judicial_case_search', locked_name, server='case', search_param='keyword', extra_args={'case_type': '民事案件', 'size': 20})
    result['被诉历史'] = {'_count': _count_items(r), '_summary': f'{_count_items(r)}件'}
    return result

def _stage_h_calc_metrics(stage_b: dict, stage_c: dict, stage_d: dict, stage_e: dict, stage_f: dict, stage_g: dict) -> dict:
    """计算最终指标：回款概率、判赔调整系数、时间延量"""
    metrics = {'recovery_probability': 50.0, 'damages_adjustment': '基准', 'time_extra_months': 0, 'red_flags': [], 'green_flags': [], 'details': {}}
    prob = 50.0
    ...
    channel_count = sum((stage_f.get(k, {}).get('_count', 0) for k in ('线上店铺', 'APP信息', '小程序', '微信公众号', '抖音账号')))
    ...
    extra_months = 0
    if case_count > 0:
        extra_months += min(case_count, 10)
    if terminated_c > 0:
        extra_months += terminated_c * 3
...
def _search_individual(d: dict) -> Dict:
    ...
    dishonest = _safe_call('get_executive_dishonest', pname, server='executive', extra_args={'personName': pname})
    if _count_items(dishonest) > 0:
        result['metrics']['red_flags'].append(f'失信被执行人')
        result['metrics']['recovery_probability'] *= 0.1
    debtor = _safe_call('get_executive_judgment_debtor', pname, server='executive', extra_args={'personName': pname})
    jd_count = _count_items(debtor)
    if jd_count >= 3:
        result['metrics']['red_flags'].append(f'被执行人≥{jd_count}次')
        result['metrics']['recovery_probability'] *= 0.3
    high_consume = _safe_call('get_executive_high_consumption_ban', pname, server='executive', extra_args={'personName': pname})
    if _count_items(high_consume) > 0:
        result['metrics']['red_flags'].append('限高消费')
    result['stages']['E_失信被执行限高'] = {'失信': _count_items(dishonest), '被执行': jd_count, '限高': _count_items(high_consume)}
    controlled = _safe_call('get_executive_controlled_companies', pname, server='executive', extra_args={'personName': pname})
    ...
    eq_freeze = _safe_call('get_executive_equity_freeze', pname, server='executive', extra_args={'personName': pname})
    if _count_items(eq_freeze) > 0:
        result['metrics']['red_flags'].append('股权冻结')
    eq_pledge = _safe_call('get_executive_equity_pledge', pname, server='executive', extra_args={'personName': pname})
    if _count_items(eq_pledge) > 0:
        result['metrics']['red_flags'].append('股权质押')
    result['stages']['E_资产状况'] = {'控制企业': cc_count, '股权冻结': _count_items(eq_freeze), '股权质押': _count_items(eq_pledge)}
    result['metrics']['recovery_probability'] = round(max(min(result['metrics']['recovery_probability'], 100), 0), 1)
