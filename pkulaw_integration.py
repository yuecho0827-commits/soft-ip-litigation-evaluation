...
def generate_all_queries(case_id: str, case_description: str, deepseek_results: Dict) -> Dict:
    ...
    返回结构化查询计划
    """
    queries = {'case_id': case_id, 'generated_at': datetime.now().isoformat(), 'dimensions': {}}
    rights = deepseek_results.get('rights', {})
    queries['dimensions']['1.1_权利基础'] = {'label': '权利基础', 'searches': [{'id': 'law_trademark', 'type': 'law_search', 'tool': 'search_article', 'query': '商标法 商标注册 注册商标 有效期 续展 撤三', 'lib': '中央', 'size': 5, 'purpose': '检索商标法核心法条，验证商标权利基础'}, {'id': 'law_wellknown', 'type': 'law_keyword', 'tool': 'get_law_list', 'title': '驰名商标认定和保护规定', 'fulltext': '驰名商标 认定 跨类保护', 'purpose': '检索驰名商标相关法规'}]}
    infringement = deepseek_results.get('infringement', {})
    queries['dimensions']['1.2_侵权认定'] = {'label': '侵权认定', 'searches': [{'id': 'case_infringement', 'type': 'case_search', 'tool': 'search_case', 'query': '商标侵权 近似商标 混淆可能性 相同商品 类似商品', 'case_type': '民事案件', 'size': 5, 'purpose': '检索商标侵权类案，验证侵权认定标准'}]}
    queries['dimensions']['1.3_诉讼程序'] = {'label': '诉讼程序', 'searches': [{'id': 'law_limitation', 'type': 'law_search', 'tool': 'search_article', 'query': '民事诉讼 诉讼时效 管辖 仲裁协议 主体适格', 'lib': '中央', 'size': 5, 'purpose': '检索诉讼程序相关法条'}]}
    queries['dimensions']['2.1_财务回报'] = {'label': '财务回报', 'searches': [], 'note': '暂不通过北大法宝检索，后续接入企查查'}
    queries['dimensions']['2.2_判例价值'] = {'label': '判例价值', 'searches': [{'id': 'case_precedent', 'type': 'case_search', 'tool': 'search_case', 'query': '商标侵权 首案 新型侵权 指导性案例', 'case_type': '民事案件', 'size': 10, 'purpose': '检索同类在先判决，判断首案潜力'}]}
    queries['validation'] = {'label': '法条与案号验证', 'steps': [{'id': 'validate_provisions', 'tool': 'adjust_provisions', 'purpose': '验证评估报告中引用的法条是否真实有效、是否现行有效'}, {'id': 'recognize_laws', 'tool': 'law_recognition', 'purpose': '识别评估报告中的法规引用，核验准确性'}, {'id': 'recognize_cases', 'tool': 'anhao_recognition', 'purpose': '识别评估报告中的案号引用，验证真实性'}]}
...
def get_validation_status(case_id: str) -> Dict:
    ...
    return {'laws_validated': bool(results.get('law_recognition')), 'cases_validated': bool(results.get('anhao_recognition')), 'provisions_validated': bool(results.get('adjust_provisions')), 'total_sources': len(results.get('sources', [])), 'updated_at': results.get('updated_at', '')}
